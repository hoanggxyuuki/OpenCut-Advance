...
import { AudioExtractor } from "@/components/audio-extractor";
import { Image, Music, Plus, Upload, Video, Download } from "lucide-react";
...
export function MediaView() {
  ...
  const handleAudioExtracted = async (audioFile: File, audioUrl: string, duration: number) => {
    if (!activeProject) {
      toast.error("No active project");
      return;
    }

    // Add audio to media store
    const audioItem = {
      name: audioFile.name,
      type: "audio" as const,
      file: audioFile,
    ...
    await addMediaItem(activeProject.id, audioItem);
  };
  ...
  const renderPreview = (item: MediaItem) => {
    ...
    if (item.type === "video") {
          ...
          <div className="relative w-full h-full group">
            ...
            <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <AudioExtractor
                ...
                className="h-6 w-6 p-0 bg-black/70 hover:bg-black/90 border-none"
                variant="secondary"
                size="sm"
              />
            </div>
      ...
      return (
        <div className="w-full h-full bg-muted/30 flex flex-col items-center justify-center text-muted-foreground rounded relative group">
